...
class CustomMemberRepositoryImpl(
    ...
    private fun nameEq(name: String?): BooleanExpression? =
        if(!name.isNullOrEmpty()) member.memberName.contains(name) else null

    private fun gradeEq(grade: String?): BooleanExpression? =
        if(!grade.isNullOrEmpty()) member.stuNum.startsWith(grade) else null

    private fun classNumEq(classNum: String?): BooleanExpression? =
        if(!classNum.isNullOrEmpty()) member.stuNum.substring(1, 2).eq(classNum) else null

    private fun genderEq(gender: String?): BooleanExpression? =
        if(!gender.isNullOrEmpty()) member.gender.eq(Gender.valueOf(gender)) else null

    private fun roleEq(role: String?): BooleanExpression? =
        if(!role.isNullOrEmpty()) member.roles.any().eq(Role.valueOf(role)) else null
